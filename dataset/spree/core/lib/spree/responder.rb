module Spree
  class Responder < ::ActionController::Responder #:nodoc:
    ...
    def to_html
      ...
      #nodyna <instance_exec-2556> <IEX COMPLEX (block without parameters)>
      #nodyna <instance_exec-2557> <IEX COMPLEX (block without parameters)>
      has_errors? ? controller.instance_exec(&on_failure) : controller.instance_exec(&on_success)
    end

    def to_format
      super and return if !(on_success || on_failure)
      #nodyna <instance_exec-2558> <IEX COMPLEX (block without parameters)>
      #nodyna <instance_exec-2559> <IEX COMPLEX (block without parameters)>
      has_errors? ? controller.instance_exec(&on_failure) : controller.instance_exec(&on_success)
    end
